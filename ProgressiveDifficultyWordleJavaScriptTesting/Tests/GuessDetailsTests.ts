...
describe("GuessDetails", () => {
    ...
    describe("#populateCharacterStates", () => {
        it('fills character states array entirely with ExactMatch status when input maps are identical.', () => {
            let gd = new GuessDetails('alert', 'alert');
            gd.characterStates = new Array<LetterStatus>(5);
            let guessMap = new Map<string, number[]>();
        ...
        it('fills character states array entirely with WrongLocation status when input maps have no intersection.', () => {
            ...
            for (let i = 0; i < 5; i++) {
                assert.equal(LetterStatus.WrongLocation, gd.characterStates[i]);
            }
        });

        it('fills character states array entirely with all statuses to accurately represent intersection.', () => {
            let gd = new GuessDetails('risen', 'paver');
            gd.characterStates = new Array<LetterStatus>(5);
            let guessMap = new Map<string, number[]>();
            let answerMap = new Map<string, number[]>();
            ...
            assert.equal(LetterStatus.WrongLocation, gd.characterStates[0]);
            assert.equal(LetterStatus.Absent, gd.characterStates[1]);
            assert.equal(LetterStatus.Absent, gd.characterStates[2]);
            assert.equal(LetterStatus.ExactMatch, gd.characterStates[3]);
            assert.equal(LetterStatus.Absent, gd.characterStates[4]);
        });
